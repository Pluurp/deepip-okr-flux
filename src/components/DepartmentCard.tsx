...
const DepartmentCard = ({
  ...
  return (
    <Link to={`/departments/${department.id}`}>
      <Card className={cn(
        "overflow-hidden transition-all duration-300 hover:shadow-plane-md hover:-translate-y-1 bg-white border border-gray-200 shadow-plane", 
        className
      )}>
        ...
        <CardContent className="pt-4">
          <div className="grid grid-cols-2 gap-4 text-sm mb-4">
            <div>
              <p className="text-deepip-gray">Days remaining</p>
              <p className="font-medium">
                {stats.daysRemaining}/{stats.totalDays}
              </p>
            </div>
            <div className="text-right">
              <p className="text-deepip-gray text-right">&nbsp;</p>
              <p className="font-medium">&nbsp;</p>
            </div>
            <div>
              <p className="text-deepip-gray">Time progress</p>
              <p className="font-medium"></p>
            </div>
            <div className="text-right">
              <p className="text-deepip-gray text-right">&nbsp;</p>
            ...
            <div>
              <p className="text-deepip-gray">Overall progress</p>
              <p className="font-medium"></p>
            </div>
            <div className="text-right">
              <p className="text-deepip-gray text-right">&nbsp;</p>
            ...
            {objectives.length > 0 ? (
              <div className="space-y-3">
                {objectives.map((objective, index) => (
                  <div key={index} className="flex justify-between items-center p-2 rounded-md hover:bg-deepip-light-gray transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div 
                        className="w-16 h-6 rounded-full flex items-center justify-center text-xs font-medium"
                        style={{ 
              ...
              <div className="flex flex-col items-center justify-center py-4 space-y-2 border border-dashed rounded-md bg-deepip-light-gray/50">
                <p className="text-sm text-deepip-gray">No objectives defined</p>
                {onAddObjective && (
                  <Button 
                    variant="outline" 
                    size="sm" 
